...
- `plot_curves_annotated`: curves plus reference lines for theoretical
  peak throughput, streaming-only throughput, and unloaded latency,
  plus a summary text panel.
...
def extract_curves(
    ...
    spec: Spec,
    *,
    latency_measure_mode: str = "random-probe",
) -> dict:
    """Convert raw sweep results into per-read-ratio lat-tp curves.

    Returns: {
        read_ratio: {
            "throughput": [float, ...],  # GB/s, sorted low-throughput first
            "lat":        [float, ...],  # ns
            "nops":       [int, ...],    # corresponding NOP values
        }
    }
    """
    if latency_measure_mode == "random-probe":
        latency_stat = "avg_probe_latency"
    elif latency_measure_mode == "stream-only":
        latency_stat = "avg_stream_latency"
    else:
        raise ValueError(
            "latency_measure_mode must be 'random-probe' or 'stream-only', "
            f"got {latency_measure_mode!r}"
        )
    ...
    for rr, entries in sorted(by_rr.items()):
        # Sort by NOP descending so the lowest-throughput point comes first.
        entries.sort(key=lambda x: -x[0])
        throughput_list, lat_list, nop_list = [], [], []
        for nop, stats in entries:
            lat_cycles = stats["frontend"][latency_stat]

            throughput = (
                stats["memory_system"]["controller"]["total_throughput_MBps"] / 1000.0
            )
        ...
        curves[rr] = {"throughput": throughput_list, "lat": lat_list, "nops": nop_list}
...
def _draw_base_plot(curves, cfg: dict):
    ...
    ax.set_ylabel("DRAM Access Latency (ns)", fontsize=16, labelpad=8)
...
def plot_curves_annotated(
    ...
    *,
    ...
    `throughput_result` may be either a `check_peak_throughput` result (no refresh)
    or a `check_refresh_limited_throughput` result; the latter has a
    `max_achievable_throughput` key, which takes precedence as the peak reference.
    `streaming_result` is optional — if provided, a streaming reference
    line is drawn.
    """
    fig, ax = _draw_base_plot(curves, cfg)
    x_refs = [throughput_result["max_theoretical_throughput"]]

    ax.axvline(
        throughput_result["max_theoretical_throughput"],
        color="#cc7722",
        linestyle="--",
        linewidth=1.5,
        alpha=0.7,
    ...
    if "max_achievable_throughput" in throughput_result:
        x_refs.append(throughput_result["max_achievable_throughput"])
        ax.axvline(
            throughput_result["max_achievable_throughput"],
            color="#228833",
            linestyle="--",
            linewidth=1.5,
            alpha=0.7,
    ...
    if streaming_result is not None:
        x_refs.append(streaming_result["measured_streaming_throughput"])
        ax.axvline(
            streaming_result["measured_streaming_throughput"],
    ...
    ax.axhline(
        ...
        label="Theoretical Unloaded Latency (tRP+tRCD+tCL)",
    )
    curve_max_throughput = max(max(c["throughput"]) for c in curves.values())
    ax.set_xlim(left=0, right=max(curve_max_throughput, *x_refs) * 1.08)
    ...
    left_lines = [
        "Theoretical Unloaded Latency (tRP+tRCD+tCL): "
        f"{lat_result['expected_ns']:.1f} ns",
        f"Probed Unloaded Latency: {lat_result['measured_ns']:.1f} ns",
    ]
    right_lines = [
    ...
    if "max_achievable_throughput" in throughput_result:
        right_lines.append(
            "Refresh-Adjusted Achievable Throughput: "
