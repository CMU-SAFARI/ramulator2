...
def make_dut(*, channel_id=0, **overrides):
    """GDDR6 8Gb x16 @ 2000 (1250mV, double). Override via kwargs (e.g. nFAW=60)."""
...
def test_basic_read_and_write_sequences_respect_precharge_constraints():
    ...
    t_wr = t_act + write_dut.timings["nRCDWD"]
...
def test_gddr6_read_write_turnarounds_respect_direction_and_bankgroup():
    ...
    t_wr = max(
        t_act1 + read_to_write_dut.timings["nRCDWD"],
    ...
    t_wr = t_act0 + write_to_read_dut.timings["nRCDWD"]
